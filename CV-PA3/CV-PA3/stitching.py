...
def harris_corners(img, window_size=3, k=0.04):
    ...
    Ix_squared = np.multiply(dx, dx)
    Iy_squared = np.multiply(dy, dy)
    IxIy = np.multiply(dx, dy)

    M1 = convolve(Ix_squared, window)
    M2 = convolve(Iy_squared, window)
    M3 = convolve(IxIy, window)

    response = (np.multiply(M1, M2)-np.multiply(M3, M3)) - k*((M1 + M2)**2)
...
def simple_descriptor(patch):
    ...
    m, s = np.mean(patch), np.std(patch)
    if s == 0:
        s = 1
    norm = (patch - m)/s
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(N):
        sort_dists = sorted(dists[i, :])
        min_dists = sort_dists[:2]
        ratio = min_dists[0]/min_dists[1]
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        # Step 3
        M = np.dot(matched2, H)
        inliers = np.linalg.norm(M-matched1, axis=1)**2 < threshold
